...
# encoding = utf-8
import pika
def producter(username, password, host, port):
    ...
    for i in range(100):
        channel.basic_publish(exchange='', routing_key='paas', body='Paas platform rabbitMQ service verification')
    print("开始队列")
    #缓冲区已经flush而且消息已经确认发送到了RabbitMQ中，关闭链接
    # connection.close()


def consumer(username, password, host, port):
    credentials = pika.PlainCredentials(username, password)
    # 连接到rabbitmq服务器
    connection = pika.BlockingConnection(pika.ConnectionParameters(host, port, '/', credentials))
    channel = connection.channel()
...
if __name__ == '__main__':
    producter("admin", "111111", "210.13.50.105", "31272")
    consumer("admin", "111111", "210.13.50.105", "31272")
